...
int main(int argc, char** argv) {
	...
	//boost::shared_ptr<Volume> volumeFile = HDF5::matlabDeserialize<Volume> ("/usr/local.nfs/pas/teaching/pas/2014-2/rpi-2.hdf5");
	...
	d_invViewMatrix = cl::Buffer(context, CL_MEM_READ_WRITE, 12 * sizeof (float));
	...
	queue.enqueueWriteBuffer(d_output, true, 0, sizeOutput, h_outputGpu.data());
...
void render () {
	...
	if (runGpu) {
		...
		queue.enqueueWriteBuffer(d_invViewMatrix, true, 0, 12 * sizeof (float), invViewMatrix);

		// Call the kernel
		renderKernel.setArg<cl::Image3D>(0, d_input);
		renderKernel.setArg<cl::Buffer>(1, d_output);
		renderKernel.setArg<cl::Buffer>(2, d_invViewMatrix);
		renderKernel.setArg<float>(3, tstep);
		renderKernel.setArg<float>(4, brightness);
		queue.enqueueNDRangeKernel(renderKernel, cl::NullRange, cl::NDRange (outX, outY), cl::NullRange, NULL, &kernelExecution);

		// Copy output data back to host
		queue.enqueueReadBuffer(d_output, true, 0, sizeOutput, h_outputGpu.data(), NULL, &copyToHost);
	}
	...
	Core::TimeSpan copyTime = OpenCL::getElapsedTime(copyToHost);
	Core::TimeSpan overallGpuTime = gpuTime + copyTime;
	std::cout << "CPU Time: " << cpuTime.toString() << ", " << (countOutput / cpuTime.getSeconds() / 1e9) << " GPixel/s" << std::endl;;
	if (runGpu) {
		std::cout << "Memory copy Time: " << copyTime.toString() << std::endl;
		std::cout << "GPU Time w/o memory copy: " << gpuTime.toString() << " (speedup = " << (cpuTime.getSeconds() / gpuTime.getSeconds()) << ", " << (countOutput / gpuTime.getSeconds() / 1e9) << " GPixel/s)" << std::endl;
		std::cout << "GPU Time with memory copy: " << overallGpuTime.toString() << " (speedup = " << (cpuTime.getSeconds() / overallGpuTime.getSeconds()) << ", " << (countOutput / overallGpuTime.getSeconds() / 1e9) << " GPixel/s)" << std::endl;
	}
